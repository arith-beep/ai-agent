...
import { toolsRepo, knowledgeRepo, tenancyRepo } from "@ai-agent/storage";
import { AgentForm } from "../_components/agent-form";

export default async function NewAgentPage() {
  const ctx = await requireCurrentContext();
  const [tools, knowledgeBases, orgMembers] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      <AgentForm tools={tools} knowledgeBases={knowledgeBases} orgMembers={orgMembers} />
    </div>
  );
}
