...
export async function setManagerAgentConfigAction(formData: FormData) {
  ...
  const modelProvider = String(formData.get("modelProvider") ?? "") as "openai" | "anthropic" | "google";
  const modelName = String(formData.get("modelName") ?? "").trim();
  if (!modelProvider || !modelName) {
    redirect("/manager?error=" + encodeURIComponent("Choose a provider and model name."));
  }
  await managerRepo.setManagerAgentConfig({ orgId: ctx.orgId, modelProvider, modelName, updatedByUserId: ctx.userId });
