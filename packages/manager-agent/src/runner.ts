...
function handleGenerationError(error: unknown, phase: "evidence gathering" | "brief synthesis"): { status: "error"; message: string } {
  const { userMessage, logDetail } = describeGenerationError(error, phase);
...
export async function generateMorningBrief(orgId: string): Promise<GenerationResult<MorningBrief>> {
  ...
  try {
    ...
    return handleGenerationError(error, "evidence gathering");
  }
  ...
  try {
    ...
    return handleGenerationError(error, "brief synthesis");
  }
}
